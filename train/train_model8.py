...
from models.model_8 import VisionTransformer
from data.data_loaders import get_data_loaders
from utils.metrics import evaluate_model, save_confusion_matrix

def train_model_8(epochs=100, patience=5, lr=1e-4, weight_decay=1e-4):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Load filtered data with histogram + bilateral preprocessing
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            patience_counter = 0
            torch.save(model.state_dict(), "best_model_8_vit.pth")
    ...
    # Final evaluation
    model.load_state_dict(torch.load("best_model_8_vit.pth"))
    model.eval()
    evaluate_model(model, test_loader, device, class_names=class_names, show_report=True)
    save_confusion_matrix(model, test_loader, class_names, "confusion_matrix_model_8_vit.png", device=device)
